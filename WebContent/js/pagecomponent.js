...
function myonclick(){
	...
	document.getElementById('panel').innerHTML=`
		...
		<div class="col-md-12">
		<table class="table table-striped" id="userOperationRecordTable1"></table>
		</div>
		</div>
		</div>
		</div>
...
function myclassgrade(){
	document.getElementById('panel').innerHTML=`<div id="container" style="height: 100%"></div>`
	      document.getElementById('panel').style="height:800px";
	...
	$.ajax({
		...
		success : function(massge, Status) {
			...
			var class_Name=new Array();
		      ...
		      var class_Number1=new Array();
		      for(var key in obj[0]){
		        for(var i=0;i<obj.length;i++){
		          if(key=="class_Name"){
		            class_Name[i]=obj[i][key];
		          }else if (key=="class_Number") {
		            class_Number[i]=obj[i][key];
		          }else if (key=="class_Number1") {
		            class_Number1[i]=obj[i][key];
		          }
		        }
		      }
		      var class_Number2=new Array();
		      for(var i=0;i<class_Name.length;i++){
		        class_Number2[i]=class_Number[i]-class_Number1[i];
		      }
...
function myclick() {
	document.getElementById('panel').innerHTML=`
		...
		<span class="glyphicon glyphicon-search"></span> <span>期末成绩查询</span>
		</button>
		</div>
		</div>
		<div class="row" style="margin-top:25px">
